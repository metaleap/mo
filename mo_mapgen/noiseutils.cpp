...
void WriterBMP::WriteDestFile() {
    ...
    noise::uint8* pLineBuffer = nullptr;
...
void WriterTER::WriteDestFile() {
    ...
    os.write("SIZE", 4);
    os.write((char*)UnpackLittle16(d, GetMin((noise::uint16)(width), (noise::uint16)(height)) - 1), 2);
    os.write("\0\0", 2);
    os.write("XPTS", 4);
    os.write((char*)UnpackLittle16(d, (noise::uint16)(width)), 2);
    os.write("\0\0", 2);
    os.write("YPTS", 4);
    os.write((char*)UnpackLittle16(d, (noise::uint16)(height)), 2);
    os.write("\0\0", 2);
    os.write("SCAL", 4);
...
NoiseMapBuilder::NoiseMapBuilder()
    : m_pCallback(nullptr), m_destHeight(0), m_destWidth(0), m_pDestNoiseMap(nullptr), m_pSourceModule(nullptr) {
}
...
NoiseMapBuilderCylinder::NoiseMapBuilderCylinder()
    : m_lowerAngleBound(0.0), m_lowerHeightBound(0.0), m_upperAngleBound(0.0), m_upperHeightBound(0.0) {
}

void NoiseMapBuilderCylinder::Build() {
    if (m_upperAngleBound <= m_lowerAngleBound || m_upperHeightBound <= m_lowerHeightBound || m_destWidth <= 0
        || m_destHeight <= 0 || m_pSourceModule == nullptr || m_pDestNoiseMap == nullptr) {
        throw noise::ExceptionInvalidParam();
    }
...
NoiseMapBuilderPlane::NoiseMapBuilderPlane()
    : m_isSeamlessEnabled(false), m_lowerXBound(0.0), m_lowerZBound(0.0), m_upperXBound(0.0), m_upperZBound(0.0) {
}

void NoiseMapBuilderPlane::Build() {
    if (m_upperXBound <= m_lowerXBound || m_upperZBound <= m_lowerZBound || m_destWidth <= 0 || m_destHeight <= 0
        || m_pSourceModule == nullptr || m_pDestNoiseMap == nullptr) {
        throw noise::ExceptionInvalidParam();
    }
...
NoiseMapBuilderSphere::NoiseMapBuilderSphere()
    : m_eastLonBound(0.0), m_northLatBound(0.0), m_southLatBound(0.0), m_westLonBound(0.0) {
}

void NoiseMapBuilderSphere::Build() {
    if (m_eastLonBound <= m_westLonBound || m_northLatBound <= m_southLatBound || m_destWidth <= 0 || m_destHeight <= 0
        || m_pSourceModule == nullptr || m_pDestNoiseMap == nullptr) {
        throw noise::ExceptionInvalidParam();
    }
...
RendererImage::RendererImage()
    : m_isLightEnabled(false), m_isWrapEnabled(false), m_lightAzimuth(45.0), m_lightBrightness(1.0),
      m_lightColor(255, 255, 255, 255), m_lightContrast(1.0), m_lightElev(45.0), m_lightIntensity(1.0),
      m_pBackgroundImage(nullptr), m_pDestImage(nullptr), m_pSourceNoiseMap(nullptr), m_recalcLightValues(true) {
...
Color RendererImage::CalcDestColor(const Color &sourceColor, const Color &backgroundColor, double lightValue) const {
    ...
    // the new color.
    Color newColor((noise::uint8)((noise::uint)(red * 255.0) & 0xff), (noise::uint8)((noise::uint)(green * 255.0) & 0xff),
                   (noise::uint8)((noise::uint)(blue * 255.0) & 0xff), GetMax(sourceColor.alpha, backgroundColor.alpha));
    return newColor;
}
...
void RendererImage::Render() {
    if (m_pSourceNoiseMap == nullptr || m_pDestImage == nullptr || m_pSourceNoiseMap->GetWidth() <= 0
    ...
    for (int y = 0; y < height; y++) {
        const Color* pBackground = nullptr;
        if (m_pBackgroundImage != nullptr) {
...
RendererNormalMap::RendererNormalMap()
    : m_bumpHeight(1.0), m_isWrapEnabled(false), m_pDestImage(nullptr), m_pSourceNoiseMap(nullptr) {};
