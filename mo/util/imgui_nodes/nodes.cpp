// 23/12/11 from https://gist.github.com/ChemistAion/0cd64b71711d81661344af040c142c1c#file-nodes-cpp
...
ImGuiNodesNode* ImGuiNodes::UpdateNodesFromCanvas() {
    ...
    for (int node_idx = nodes_.size(); node_idx != 0;) {
        ...
        if (canvas.Overlaps(node_rect)) {
            ...
            node->state_ &=
                ~(ImGuiNodesNodeStateFlag_Visible | ImGuiNodesNodeStateFlag_Hovered | ImGuiNodesNodeStateFlag_Marked);
            continue;
        }
        ...
        for (int input_idx = 0; input_idx < node->inputs_.size(); ++input_idx) {
            ...
            input.state_ &= ~(ImGuiNodesConnectorStateFlag_Hovered | ImGuiNodesConnectorStateFlag_Consider
        ...
        for (int output_idx = 0; output_idx < node->outputs_.size(); ++output_idx) {
            ...
            output.state_ &= ~(ImGuiNodesConnectorStateFlag_Hovered | ImGuiNodesConnectorStateFlag_Consider
                               | ImGuiNodesConnectorStateFlag_Draging);
        ...
        switch (state_) {
            case ImGuiNodesState_Default: {
                ...
                for (int node_idx = 0; node_idx < nodes_.size(); ++node_idx) {
                    ...
                    state &= ~(ImGuiNodesNodeStateFlag_Selected | ImGuiNodesNodeStateFlag_Marked
...
void ImGuiNodes::ProcessNodes() {
    ...
    if (connection_.x != connection_.z && connection_.y != connection_.w)
        DrawConnection(ImVec2(connection_.x, connection_.y), ImVec2(connection_.z, connection_.w),
