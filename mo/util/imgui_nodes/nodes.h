// 23/12/11 from https://gist.github.com/ChemistAion/0cd64b71711d81661344af040c142c1c#file-nodes-h
...
struct ImGuiNodesInput {
    ...
    inline void DrawInput(ImDrawList* draw_list, ImVec2 offset, float scale, ImGuiNodesState state) const {
        ...
        if (state != ImGuiNodesState_Draging && state_ & ImGuiNodesConnectorStateFlag_Hovered
        ...
        if (state_ & (ImGuiNodesConnectorStateFlag_Consider | ImGuiNodesConnectorStateFlag_Draging))
            draw_list->AddRectFilled((area_input_.Min * scale) + offset, (area_input_.Max * scale) + offset,
                                     ImColor(0.0f, 1.0f, 0.0f, 0.5f));

        bool consider_fill = false;
        consider_fill |= bool(state_ & ImGuiNodesConnectorStateFlag_Draging);
        consider_fill |=
            bool(state_ & ImGuiNodesConnectorStateFlag_Hovered && state_ & ImGuiNodesConnectorStateFlag_Consider);
        ...
        if (consider_fill)
            draw_list->AddCircleFilled((pos_ * scale) + offset,
                                       (ImGuiNodesConnectorDotDiameter * 0.5f) * area_name_.GetHeight() * scale, color);
        else
...
struct ImGuiNodesOutput {
    ...
    inline void DrawOutput(ImDrawList* draw_list, ImVec2 offset, float scale, ImGuiNodesState state) const {
        ...
        if (state != ImGuiNodesState_Draging && state_ & ImGuiNodesConnectorStateFlag_Hovered
            && false == (state_ & ImGuiNodesConnectorStateFlag_Consider))
            draw_list->AddRectFilled((area_output_.Min * scale) + offset, (area_output_.Max * scale) + offset,
    ...
    ImGuiNodesOutput(const char* name, ImGuiNodesConnectorType type) {
        ...
        area_output_.Min.x =
            ImGuiNodesConnectorDotPadding + ImGuiNodesConnectorDotDiameter + ImGuiNodesConnectorDotPadding;
...
struct ImGuiNodesNode {
    ...
    inline void BuildNodeGeometry(ImVec2 inputs_size, ImVec2 outputs_size) {
        ...
        area_name_.Translate(ImVec2((area_node_.GetWidth() - area_name_.GetWidth()) * 0.5f,
    ...
    inline void DrawNode(ImDrawList* draw_list, ImVec2 offset, float scale, ImGuiNodesState state) const {
        ...
        if (state_ & ImGuiNodesNodeStateFlag_Processing)
            draw_list->AddRectFilled(node_rect.Min - outline, node_rect.Max + outline, body_color, rounding,
        ...
        if (false == (state_ & ImGuiNodesNodeStateFlag_Collapsed))
            draw_list->AddLine(ImVec2(node_rect.Min.x, head.y), ImVec2(head.x - 1.0f, head.y),
                               ImColor(0.0f, 0.0f, 0.0f, 0.5f), 2.0f);

        const ImDrawFlags head_corners_flags =
            state_ & ImGuiNodesNodeStateFlag_Collapsed ? rounding_corners_flags : ImDrawFlags_RoundCornersTop;
        ...
        if (state_ & (ImGuiNodesNodeStateFlag_Marked | ImGuiNodesNodeStateFlag_Selected))
            draw_list->AddRectFilled(node_rect.Min, node_rect.Max, ImColor(1.0f, 1.0f, 1.0f, 0.25f), rounding,
                                     rounding_corners_flags);
        ...
        if (state_ & ImGuiNodesNodeStateFlag_Processing) {
            ...
            draw_list->AddRect(node_rect.Min - outline, node_rect.Max + outline, processing_color, rounding,
                               rounding_corners_flags, 2.0f * scale);
        } else {
            draw_list->AddRect(node_rect.Min - outline * 0.5f, node_rect.Max + outline * 0.5f,
                               ImColor(0.0f, 0.0f, 0.0f, 0.5f), rounding, rounding_corners_flags, 3.0f * scale);
        }
    }
...
struct ImGuiNodes {
  ...
  private:
    ...
    inline bool ConnectionMatrix(ImGuiNodesNode* input_node, ImGuiNodesNode* output_node, ImGuiNodesInput* input,
